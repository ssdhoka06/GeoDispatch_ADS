...
# { facility_id : { "id": int, "x": float, "y": float, "name": str, "type": str } }
...
def init_from_facilities(facilities: List[dict]) -> None:
    ...
    logger.info("State manager initialised with %d facilities",
                len(_facility_states))
       ...
       and prev == FacilityState.ONLINE:
        ...
        if gd:
            try:
                ...
                # Voronoi incremental update (P5 will add to bridge)
                if hasattr(gd, 'voronoi_incremental_update'):
...
def register_new_facility(x: float, y: float,
    ...
    _facility_states[fid] = FacilityState.ONLINE
    _facility_meta[fid] = {
        "id": fid, "x": x, "y": y, "name": name, "type": ftype
    }
